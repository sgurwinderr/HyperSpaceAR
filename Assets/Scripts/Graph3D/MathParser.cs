...
public class MathParser {
    ...
    private const string VariableMarker = "~";
    ...
    private const string x = VariableMarker + "x";
    private const string y = VariableMarker + "y";
    ...
    private readonly Dictionary<string, string> supportedVariables =
        new Dictionary<string, string>
        {
                { "x", x },
                { "y", y }
    ...
    private string FormatString(string expression)
    {
        ...
        Console.WriteLine("formattedString.ToString() = {0}", formattedString.ToString());
    ...
    private string ConvertToRPN(string expression)
    {
        ...
        Console.WriteLine("outputString end {0}", outputString);
    ...
    private string LexicalAnalysisInfixNotation(string expression, ref int pos)
    {
        ...
        Console.WriteLine("token.ToString() {0}", token.ToString());
        // If it is a operator
            ...
            || supportedConstants.ContainsKey(token.ToString()))
        {
            ...
            else if (supportedVariables.ContainsKey(token.ToString()))
            {
                return supportedVariables[token.ToString()];
            }
            else
            {
    ...
    {
        this.RPN = ConvertToRPN(FormatString(exp));
    }

    public double Calculate(string x, string y)
    {
        string finalExp;
        finalExp = RPN.Replace("~x", "#" + x);
        finalExp = finalExp.Replace("~y", "#" + y);
        return Calculate(finalExp);
    }
